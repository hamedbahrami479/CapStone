
const productData = [

    {
    id: '1',
    productcategory: 'Plastic Transparent',
    handling: 'Yellow Bin',
    advice: '',
    image: require('./Picture/Yellow-Bin.png'),
    },

    {
    id:'2',
    productcategory: 'Plastic Black',
    handling: 'Yellow Bin',
    advice: 'Did you know, sorting machines are not always able to sort out black plastic. But still: Put it into the yellow bin. But, maybe you can try to avoid black plastics?',
    image: require('./Picture/Yellow-Bin.png'),
    },

    {
    id:'3',
    productcategory: 'Aluminum',
    handling: 'Yellow Bin',
    advice: '',
    image: require('./Picture/Yellow-Bin.png'),
    },

    {
    id:'4',
    productcategory: 'Aluminum Contaminated',
    handling: 'Black Bin',
    advice: '',
    image: require('./Picture/Yellow-Bin.png'),
    },

    {
    id:'5',
    productcategory:'Paper Contaminated',
    handling: 'Black Bin',
    advice: '',
    image: require('./Picture/Black-Bin.png'),
    },

    {
    id:'6',
    productcategory:'Paper Coated',
    handling: 'Black Bin',
    advice: '',
    image: require('./Picture/Black-Bin.png'),
    },

    {
    id:'7',
    productcategory:'Paper Clean',
    handling: 'Blue Bin',
    advice: '',
    image: require('./Picture/Blue-Bin.png'),
    },

    ];

    export default productData;